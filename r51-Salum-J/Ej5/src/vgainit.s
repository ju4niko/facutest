SECTION .video progbits

GLOBAL	VGA_Init

; Configuración de PCI en Bochs
; Hecho por Dario Alpern el 6 de junio de 2015.
; Mayor informacion en http://www.osdever.net/FreeVGA/vga/vgareg.htm
;-------------------------------------------------------------------------------
;|  Titulo: VGA_Init                                                           |
;|  Versión:        1.0                     Fecha:  06/06/2015                 |
;|  Autor:          D. Alpern               Modelo: IA-32 (16/32bits)          |
;|  ------------------------------------------------------------------------   |
;|  Descripción:                                                               |
;|      Inicializa el controlador VGA                                          |
;|  ------------------------------------------------------------------------   |
;|  Recibe:                                                                    |
;|      Nada                                                                   |
;|                                                                             |
...
;|  ------------------------------------------------------------------------   |
;|  Revisiones:                                                                |
;|      1.0 | 06/06/2015 | D. Alpern | Original                                |
;|      1.1 | 01/06/2017 | ChristiaN | Se implementa como funcion              |
;-------------------------------------------------------------------------------
...
;
; Recorrer bus PCI. 
; Bochs solo soporta bus cero ya que no tiene "PCI to PCI bridge".
...
;
recorrer_bus_pci:
   mov bp, 0x4000             ;Primera dirección de entrada/salida para PCI.
...
dispositivo_hallado:
                              ;Leer registro 0x0C: Bit 23: multiple function bit
...
ir_a_siguiente_funcion:
   ...
   jmp ciclo_recorrer_bus_pci
funcion_hallada:
                              ;Leer registro 0x0C: Bits 22-16: header type
   leer_registro_pci 0x0C
...
habilitar_modo_texto:
   ...
   ;-> Establecer los registros SEQ <-
   cld
   mov cl, 0
   mov si, seq_regs_data

   seq_regs_loop:
      mov dx, 0x3C4           ;Puerto del secuenciador VGA (indice)
      mov al, cl
      out dx, al
      ...
      inc cl                  ;Siguiente indice.
      cmp cl, 5
      jne seq_regs_loop

   ;Eliminar protección de los registros 0 a 7 del controlador VGA.
   mov ax, 0x2C11
   mov dx, 0x3D4              ;Puerto del controlador VGA (indice VGA_CRTC_ADDR)
   out dx, ax

   ;-> Establecer los registros CRTC <-
   mov cl, 0
   mov si, crtc_regs_data
   crtc_regs_loop:
      mov dx, 0x3D4           ;Puerto del controlador VGA (indice)
      mov al, cl
      out dx, al
      cs lodsb
      inc dx                  ;Puerto del controlador VGA (datos)
      out dx, al
      inc cl
      cmp cl, 25
      jb crtc_regs_loop
...
inicializar_paleta:
   ...
   inicializar_paleta_loop:
      mov dx, 0x3C8           ;Puerto de indice paleta
   ...
   ;-> Establecer los registros del controlador grafico VGA <-
   mov cl, 0
   mov si, graphics_regs_data
graphics_regs_loop:
   mov dx, 0x3CE              ;Puerto del controlador grafico VGA (indice)
   mov al, cl
   out dx, al

   cs lodsb
   inc dx                     ;0x3CF
   out dx, al                 ;Puerto del controlador grafico VGA (datos)

   inc cl
   cmp cl, 9
   jb graphics_regs_loop

   ;-> Establecer el registro de estado para emulacion de color <-
   mov dx, 0x3DA              ;Puerto de estado para emulacion de color
   in al, dx

   mov cl, 0
   mov si, attrcon_regs_data
attrcon_regs_loop:
   mov dx, 0x3C0              ;Registro de atributos del controlador
   mov al, cl                 ;Escribir número de registro
   out dx, al
   cs lodsb                   ;Leer dato a cargar en el registro.
   out dx, al                 ;Cargar dato en el registro.
   ...
   cmp cl, 21                 ;Ver si se escribieron todos los registros
   jb attrcon_regs_loop       ;Saltar si no es asi.

   mov al, 0x20               ;Reactivar el refresco de pantalla porque la
   out dx, al                 ;escritura a registros de ATTRCON lo desactiva.

   mov dx, 0x03ce             ;Puerto del controlador grafico VGA (indice)
   mov ax, 5                  ;borrar modo par/impar
   out dx, ax
...
crtc_regs_data:
   ...
   db 0x4F  ;Bits 4-0: máxima linea de scan.
   ...
   db 0xFF  ;Registro de comparación de lineas.
...
attrcon_regs_data:
   ...
   db 0x0C  ;Registro de control de atributos.
            ...
            ;* Bit 5: Modo de desplazamiento de pixeles
            ;* Bit 4: No usado
   ...
   db 0x08  ;Desplazamiento de pixeles de video hacia la izquierda.
