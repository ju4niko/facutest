...
global	timer_handler    ;IRQ 32
global 	irq_keyb_handler	; irq33 handler del teclado
...
%define GRIS 0X07						; Codigo de color GRIS
...
timer_handler:
	...
	update_screenValue: 							; Actualizamos pantalla
		...
		mov al, 0x20
    	out 0x20, al 						; Habilitamos nuevamente el PIC
...
pushad 									; Salvamos registros
	...
	in 	al , 0x60  							; Lee teclado guarda en EAX

	;-------------------------------------------
	; Checkeamos MAKECODE o BREAKCODE...
	;-------------------------------------------
	cmp eax, 0x81 				 ;limite para saber si es Break o make
	jge BREAKCODE				; si es Break
	jl  MAKECODE				; si es make lo tomo 
	...
	MAKECODE:	
		...
		insertBufer:
			;tenesmoq eu guardar la tecla pulsada
			;xchg	bx, bx														;magic breakpoint	
			...
			call scanCode2ascii
			mov edx,eax
			mov word [charValue], dx 
			;mov [charValue],edx
			pop eax
						;imprimo la letra pulsada
								push VERDE 							; Color
								push 0x05 							; Fila
								push 0x00 							; Columna
								push edx 	     			; Mensaje
			...
			;push eax 						; Tecla a insertar en la FIFO	
			;call put_fifo 		 			; Funcion PUSH_FIFO
			;add  esp, 4 					; Borramos parametro

			jmp keyb_handler_end
	...
	BREAKCODE:
		...
		;-------------------------------------------
		; Chqueamos si es BC SHIFT (unico que interesa)...
		;-------------------------------------------
		cmp eax, 0x2A		 				; Checkeamos si es  SHIFT
   		je  insertBufer				 		; Si es... insertamos	
